...
1. **Azure CLI**
   ...
   - Login to your specific cloud before running a script:
     ```
...
- The scripts run **only in the customer’s Azure tenant**
...
```bash
cd Impact-Analyzer
chmod +x Get-RetirementImpactedResources.sh
./Get-RetirementImpactedResources.sh
```

### Recommended place to test the shell script

For the Bash shell script, **Azure Cloud Shell** is recommended for first-time validation because Bash and Azure CLI are already available and you can run the script directly after uploading the files.
...
- The scripts require explicit user consent before execution
